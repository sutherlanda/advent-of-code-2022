...
run = do
  ...
  let completeModel = dropUntilAbyss modelWithRocks
  printModel completeModel
  putStrLn ""
...
getModelBounds rockCoords = ((minX, maxX), (0, maxY))
  where
    minX = minimum (map fst rockCoords)
    maxX = maximum (map fst rockCoords)
    maxY = maximum (map snd rockCoords)

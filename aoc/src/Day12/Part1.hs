...
run :: IO ()
run = do
  putStrLn "Running Day 12, Part 1 solution..."
  grid <- readInputFile
  let startCoords = fromJust $ getCoords 'S' grid
  let endCoords@(x, y) = fromJust $ getCoords 'E' grid
  let distances = updateMatrix 0 startCoords $ replicate (length grid) (replicate (length (head grid)) (maxDistance grid))
  let visited = replicate (length grid) (replicate (length (head grid)) False)
  let shortestPaths = dijkstras grid visited distances
  print $ shortestPaths !! y !! x
  where
